...
double black_scholes_price(
    ...
    if (spot <= 0.0 || strike <= 0.0 || time_to_maturity < 0.0 || volatility < 0.0) {
        return 0.0;
    }

    if (time_to_maturity <= kEpsilon) {
    ...
    if (volatility <= kEpsilon) {
        const double forward = spot / discount_factor;
        return discount_factor * intrinsic_value(forward, strike, option_type);
    }
